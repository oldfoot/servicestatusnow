CREATE DEFINER=`servicestatusnow`@`%` PROCEDURE `sp_service_category_browse`(IN pAPIAuthCode VARCHAR(100),IN pOrganisationID INT(5))
BEGIN
	...
	IF (@count_priv = 1) THEN
		...
		IF (@count_priv1 = 1) THEN			
			SELECT CategoryID, CategoryName FROM service_category_master
			WHERE OrganisationID = pOrganisationID;			
		END IF;
	END IF;
    END