CREATE DEFINER=`servicestatusnow`@`%` PROCEDURE `sp_service_status_master_add`(IN pAPIAuthCode VARCHAR(100),IN pServiceID INT(5),IN pServiceCode INT(5),IN pServiceDesc VARCHAR(255))
BEGIN
	...
	IF (@count_priv = 1) THEN
		...
		IF (@count_priv1 = 1) THEN
			...
			IF (@count_priv2 = 1) THEN
				UPDATE service_status_master SET DateTimeUpdated = SYSDATE() WHERE ServiceID = pServiceID;
				INSERT INTO service_status_master_log (ServiceID,ServiceCode,ServiceDesc,DateTimeAdded,DateTimeUpdated,UserIDUpdated)
				SELECT ServiceID,ServiceCode,ServiceDesc,DateTimeAdded,DateTimeUpdated,UserIDUpdated FROM service_status_master WHERE ServiceID = pServiceID;
				DELETE FROM service_status_master WHERE ServiceID = pServiceID;
				INSERT INTO service_status_master(ServiceID,ServiceCode,ServiceDesc,DateTimeAdded,UserIDUpdated)
