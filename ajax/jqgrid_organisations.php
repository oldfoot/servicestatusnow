...
$sql = "SELECT COUNT(*) AS count FROM core_organisation_master";
//echo $sql;
$result = $db->Query($sql);
...
$SQL = "SELECT OrganisationID, OrganisationName, AccountType
					FROM core_organisation_master
					";
$result = $db->Query($SQL);
...
while($row = $db->FetchArray($result,MYSQL_ASSOC)) {
	$count++;
	echo "<row id='".$row['OrganisationID']."'>\n";
	echo "<cell>".$row['OrganisationID']."</cell>\n";
	//echo "<cell>". $count."</cell>\n";	
	echo "<cell>". $row['OrganisationName']."</cell>\n";		
	echo "<cell>". $row['AccountType']."</cell>\n";	
	echo "</row>\n";
}
echo "</rows>\n";
?>